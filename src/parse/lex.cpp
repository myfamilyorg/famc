...
Lexer::Lexer(const ::std::string& filename, AST::Edition edition, ParseState ps)
    : TokenStream(ps),
      ...
      m_line(1),
      m_line_ofs(0),
      m_istream_fp(filename != "-" ? new std::ifstream(filename.c_str())
				   : nullptr),
		...
		if (this->getc_byte() == '\xef') {
			if (this->getc_byte() != '\xbb') {
				throw ::std::runtime_error(
				    "Incomplete BOM - missing \\xBB in second "
				    "position");
			}
			if (this->getc_byte() != '\xbf') {
				throw ::std::runtime_error(
				    "Incomplete BOM - missing \\xBF in third "
				    "position");
			}
			m_line_ofs = 0;
		} else {
...
bool issym(Codepoint ch) {
	if ('0' <= ch.v && ch.v <= '9') return true;
	if (::std::isalpha(ch.v)) return true;
	if (ch == '_') return true;
	if (ch.v >= 128) return !ch.isspace();
	return false;
}
...
Ident::Hygiene Lexer::realGetHygiene() const { return m_hygiene; }
	...
	try {
		...
		if (sym == 0) {
			...
			if (ch.isdigit()) {
				...
				if (ch == 'e' || ch == 'E' || ch == '.') {
					...
					if (ch == '.') {
						...
						// Single dot followed by a
						// non-digit, could be a float
						// or an integer with a
						// method/field access NOTE:
						// `1.e1` is not a float
						if (!ch.isdigit()) {
							this->ungetc();
							if (issym(ch)) {
					...
					if (num_mode != NumMode::DEC)
						TODO(this->point_span(),
						     "Non-decimal floats");

					this->ungetc();
					double fval = this->parseFloat(val);
					if (fval != fval) {
						assert(!this->m_next_tokens
							    .empty());
						auto t = std::move(
						    this->m_next_tokens.back());
			...
			// Byte/Raw strings
				...
				if (ch == 'r') {
					return this->getTokenInt_RawString(
					    is_byte);
				} else {
					assert(is_byte);
								...
								if (v != ~0u) {
									if (v >
										...
										throw ParseError::
										    Generic(
											*this,
											"Value out of range for byte literal");
									str +=
									    (char)
										v;
								}
			...
			// If the symbol is `TOK_DOT`, check if the next
			// character is a digit and consume an integer
			if (sym == TOK_DOT) {
				auto ch = this->getc();
				this->ungetc();
				if (ch.isdigit()) {
					auto val = this->parseInt(nullptr);
					m_next_tokens.push_back(
					    Token(val, CORETYPE_ANY));
				} else {
				}
			}
			...
			switch (sym) {
				case LINECOMMENT: {
					...
					if (is_doc || is_pdoc) {
						...
						m_next_tokens.push_back(
						    TOK_SQUARE_CLOSE);
						m_next_tokens.push_back(
						    Token(TOK_STRING, mv$(str),
							  realGetHygiene()));
						m_next_tokens.push_back(
						    TOK_EQUAL);
						m_next_tokens.push_back(Token(
						    TOK_IDENT,
						    RcString::new_interned(
							"doc")));
						m_next_tokens.push_back(
						    TOK_SQUARE_OPEN);
						if (is_pdoc)
				...
				case SINGLEQUOTE: {
					...
					if (firstchar.v == '\\') {
						...
						return Token(U128(val),
							     CORETYPE_CHAR);
					} else if (firstchar.v == '\'') {
						TODO(this->point_span(),
						     "Proper error for empty "
						     "char literals");
					} else {
						ch = this->getc();
						if (ch == '\'') {
...
Token Lexer::getTokenInt_RawString(bool is_byte) {
	...
	if (ch != '"') {
		...
		// Raw identifier
			...
			return this->getTokenInt_Identifier(
			    ch, Codepoint(), /*parse_reserved_word*/ false);
		} else {
			throw ParseError::Generic(
			    *this, "Expected '\"' after hashes following `r`");
		}
	}
	auto terminator = ch;
	::std::string val;
	...
	for (;;) {
		try {
			...
			throw ParseError::Generic(*this,
						  "EOF reached in raw string");
		}
...
Token Lexer::getTokenInt_Identifier(Codepoint leader, Codepoint leader2,
				    bool parse_reserved_word) {
	::std::string str;
	if (leader2 != '\0') str += leader;
	...
	return Token(TOK_IDENT, Ident(this->realGetHygiene(),
				      RcString::new_interned(str)));
}
...
U128 Lexer::parseInt(NumMode* num_mode_out) {
	...
	this->ungetc();
	if (num_mode_out) *num_mode_out = num_mode;
	return val;
}

// Takes the VERY lazy way of reading the float into a string then passing to
// strtod
double Lexer::parseFloat(U128 whole) {
	std::string sbuf = FMT(whole << ".");
	// char buf[MAX_LEN+1];
	// int ofs = snprintf(buf, MAX_LEN+1, "%llu.", (unsigned long
	// long)whole);
...
#define PUTC(ch)                                                               \
	do {                                                                   \
		assert(ch.v < 127);                                            \
		sbuf += char(                                                  \
		    ch.v); /* if( ofs < MAX_SIG ) { buf[ofs] = ch.v; ofs ++; } \
			      else { throw ParseError::Generic("Oversized      \
			      float"); } */                                    \
	} while (0)
	while (ch.isdigit()) {
		PUTC(ch);
	...
	if (ch == '.') {
		// TODO: `0.0..` should be a range, so if the next character is
		// `.`, then unget and continue
		ch = this->getc();
		if (ch == '.') {
			...
			// buf[ofs] = 0;
			// m_next_tokens.push_back(Token::make_float(::std::strtod(buf,
			// NULL), CORETYPE_ANY));
			m_next_tokens.push_back(Token::make_float(
			    ::std::strtod(sbuf.c_str(), NULL), CORETYPE_ANY));
			...
			// buf[ofs] = '\0';
			// assert( buf[ofs-1] != '.' );    // Shouldn't be
			// possible (as that would have been handled by the
			// caller as `<int> '..'`
			DEBUG(
			    "Detected double tuple indexing (trailing `.` "
			    "after a float - "
			    << sbuf << ")");
			...
			//*cit = '\0';
			//  - Push these in reverse order (as they're popped off
			//  the back)
			m_next_tokens.push_back(TOK_DOT);
			m_next_tokens.push_back(
			    Token(U128(std::strtoull(cit + 1, nullptr, 10)),
				  CORETYPE_ANY));
			m_next_tokens.push_back(TOK_DOT);
			m_next_tokens.push_back(
			    Token(U128(std::strtoull(buf, nullptr, 10)),
				  CORETYPE_ANY));
...
uint32_t Lexer::parseEscape(char enclosing) {
	...
	switch (ch.v) {
		case 'x': {
			...
			if (!ch.isxdigit())
				throw ParseError::Generic(
				    *this, FMT("Found invalid character '\\x"
					       << ::std::hex << ch.v
					       << "' in \\u sequence"));
		...
		case 'u': {
			...
			if (!req_close_brace)
				...
				throw ParseError::Generic(
				    *this,
				    "Expected terminating } in \\u sequence");
			else {
			}
			return val;
		}
		...
		default:
			throw ParseError::Todo(
			    *this, FMT("Unknown escape sequence \\" << ch));
	}
}

char Lexer::getc_byte() {
	int rv = m_istream.get();
	if (rv == EOF) throw Lexer::EndOfFile();
...
Codepoint Lexer::getc() {
	if (m_last_char_valid) {
		m_last_char_valid = false;
#ifdef TRACE_CHARS
		::std::cout << "getc(): U+" << ::std::hex << m_last_char.v
			    << " (cached)" << ::std::endl;
#endif
	} else {
...
Codepoint Lexer::getc_cp() {
	...
	if (v1 < 128) {
		...
		uint32_t outval = ((v1 & 0x0F) << 12) | ((e1 & 0x3F) << 6) |
				  ((e2 & 0x3F) << 0);
		return {outval};
		...
		uint32_t outval = ((v1 & 0x07) << 18) | ((e1 & 0x3F) << 12) |
				  ((e2 & 0x3F) << 6) | ((e3 & 0x3F) << 0);
		return {outval};
	} else {
...
void Lexer::ungetc() {
#ifdef TRACE_CHARS
	::std::cout << "ungetc(): cache U+" << ::std::hex << m_last_char.v
		    << ::std::endl;
#endif
...
::std::string& operator+=(::std::string& s, const Codepoint& cp) {
	if (cp.v < 0x80) {
		...
		throw ::std::runtime_error(
		    FMT("BUGCHECK: Bad unicode codepoint encountered - "
			<< ::std::hex << cp.v));
	}
	return s;
}
::std::ostream& operator<<(::std::ostream& os, const Codepoint& cp) {
	if (cp.v < 0x80) {
		...
		throw ::std::runtime_error(
		    "BUGCHECK: Bad unicode codepoint encountered");
	}
	return os;
}

Token Lex_FindOperator(const ::std::string& s) {
	if (s == "_") return TOK_UNDERSCORE;
